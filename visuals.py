...
def crosshair_line_chart(df, x_field, y_field, x_title, y_title, y_format, color="#00e5a0", height=300, x_format="%b %d"):
    ### LAYERS ###
    nearest = alt.selection_point(
        nearest=True, on="mouseover", fields=[x_field], empty=False
    )

    x_enc = alt.X(f"{x_field}:T", title=x_title, axis=alt.Axis(format=x_format, tickCount="day"))
    ...
    selectors = alt.Chart(df).mark_point().encode(
        ...
        opacity=alt.value(0),
        tooltip=[
            alt.Tooltip(f"{x_field}:T", title=x_title, format=x_format),
